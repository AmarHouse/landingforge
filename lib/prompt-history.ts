// Site and prompt history management for LandingForge
...
export const getPromptHistory = (): PromptHistoryEntry[] => {
  ...
  try {
    ...
    return JSON.parse(stored).map((entry: PromptHistoryEntry) => ({
      ...entry,
...
export const getSiteHistory = (): SiteHistoryEntry[] => {
  ...
  try {
    ...
    return JSON.parse(stored).map((entry: SiteHistoryEntry) => ({
      ...entry,
      timestamp: new Date(entry.timestamp)
    }));
  } catch (error) {
